...
const MapView = ({ onLocationUpdate }) => {
  ...
  const handleNavigateToHome = useCallback(() => {
    if (map && missionMetadata.homePosition) {
      map.panTo({ lat: missionMetadata.homePosition.lat, lng: missionMetadata.homePosition.lng });
      map.setZoom(18);
      console.log('🏠 Navigating to home position:', missionMetadata.homePosition.lat, missionMetadata.homePosition.lng);
    }
  }, [map, missionMetadata.homePosition]);
  ...
  return (
    <div className="w-full h-full relative">
      ...
      >
        ...
        {window.google?.maps && missionMetadata.homePosition && (
          <Marker
            position={{ lat: missionMetadata.homePosition.lat, lng: missionMetadata.homePosition.lng }}
            icon={{
            ...
            title={`Home Position\nLat: ${(missionMetadata.homePosition.lat || 0).toFixed(6)}\nLng: ${(missionMetadata.homePosition.lng || 0).toFixed(6)}`}
            zIndex={500}
          />
        )}
